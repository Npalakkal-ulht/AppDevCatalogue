using DigitalDeliveryWorkflow.Login.LoginObjects;
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (DbConnected)
            {
                ...
                else
                {
                    ...
                    if (startDiff <= 0)
                    {
                        ...
                        scheduledMaintenanceLabelText = "<i class='fas fa-plug font-Medium'></i> <span class='pl-2'>Planned maintenance is underway - <strong>You will not be able to log in</strong><a href='javascript:showModal()'><br />See more details</a></span>";
                    ...
                    else if (startDiff > 0 && startDiff <= noticeObjectDisplay.WarningMinutes)
                    {
                        ...
                        scheduledMaintenanceLabelText = string.Format("<i class='fas fa-plug font-Medium'></i> <span class='pl-2'>Planned maintenance is scheduled in <strong>{0}</strong> <a href='javascript:showModal()'><br />See more details</a></span>", warningTimeString);
                    }
                    else
                    {
                        ...
                        scheduledMaintenanceLabelText = string.Format("<i class='fas fa-info-circle font-Medium'></i><span class='pl-2'>Planned maintenance is scheduled for <strong>{0}</strong> <a href='javascript:showModal()'><br />See more details</a></span>", noticeObjectDisplay.StartDateTime.ToString(timeFormat));
                    }
            ...
            else
            {
                ...
                if (noticeObjectDisplay == null)
                {
                    ...
                    scheduledMaintenanceLabelText = "<i class='fas fa-exclamation'></i><span class='pl-2'>Unplanned system error occurred <a href='javascript:showModal()'><br />See more details</a></span>";
                    showSystemUnavailable = true;
                }
                else
                {
            ...
            using (HostingEnvironment.Impersonate())
            {
                ...
                using (PrincipalContext pc = new PrincipalContext(ContextType.Domain, loginCred.Domain))
                {
                    using (UserPrincipal userPrincipal = UserPrincipal.FindByIdentity(pc, IdentityType.SamAccountName, loginCred.Username))
                    {
                        ...
                        {
                            DisplayLoginErrors(LoginErrorType.CredentialsIncorrect); //account not found could return error numbers here [PRE-CHECK]
